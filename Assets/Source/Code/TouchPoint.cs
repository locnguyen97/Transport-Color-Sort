...
public class TouchPoint : MonoBehaviour
{
    ...
    public void Move()
    {
        ...
        var targetPos = GameManager.Instance.GetCurLevel().gameObjectsTarget[i].transform;
        StartCoroutine(Move(targetPos, i));
    }
    ...
    IEnumerator Move(Transform target,int index)
    {
        ...
        GameManager.Instance.GetCurLevel().gameObjectsTarget[index].SetActive(true);
        GameManager.Instance.GetCurLevel().RemoveObject(gameObject);
        GameManager.Instance.GetCurLevel().RemoveObjectTarget(target.gameObject);
    }
}